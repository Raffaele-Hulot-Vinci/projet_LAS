    ...
    while ((bytes_read = read(fd, buffer, sizeof(buffer))) > 0) {
        for (ssize_t i = 0; i < bytes_read; ++i) {
            if (buffer[i] == '\n') {
                ...
                num_lines++;
                lines = (char **)realloc(lines, (num_lines+1) * sizeof(char *));  // +1 to add NULL at the end of lines
                if (!lines) {
                    perror("Error reallocating memory");
                    return NULL;
                }
                lines[num_lines - 1] = line;
                lines[num_lines] = NULL;  // add NULL at the end of table
...
bool hostname_to_ip(const char *hostname, char *ip) {
    ...
    hints.ai_socktype = 0;           // n'importe
    hints.ai_flags = AI_PASSIVE;     // remplis mon IP pour moi
    // hints.ai_flags = AI_ADDRCONFIG; // optionnel : ignore les familles non configurées localement
    ...
    bool ok = false;
    char buf[INET6_ADDRSTRLEN]; // assez grand pour IPv6 (et donc IPv4)
    ...
    for (struct addrinfo *p = res; p != NULL; p = p->ai_next) {
        ...
        if (family == AF_INET) {
            ...
            continue; // famille non prise en charge
        }

        if (inet_ntop(family, addr, buf, sizeof(buf)) != NULL) {
            // Précondition: 'ip' a au moins INET6_ADDRSTRLEN octets
            strcpy(ip, buf);
            ok = true;
            break;
        }
    ...
    if (!ok) {
        // Soit aucune adresse retournée, soit échec de conversion
        perror("Aucune adresse IP convertible trouvée");
    }
    return ok;
}
...
int get_readable (const int* fds, const bool* fds_invalid, int nb) {
  ...
  struct pollfd* pollfds = calloc(nb, sizeof(struct pollfd));
  checkNull(pollfds, "Error: malloc in function get_readable()");
  ...
  int res = spoll(pollfds, nb, 10);
  if (res == 0) {
    return -1;
  }

  int readable_index = -1;
  for (int i=0; i<nb; i++) {
    if (pollfds[i].revents & POLLIN && !fds_invalid[i]) {
      readable_index = i;
      break;
    }
  }
